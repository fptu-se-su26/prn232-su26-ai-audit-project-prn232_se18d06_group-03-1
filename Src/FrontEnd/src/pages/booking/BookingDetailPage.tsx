import { ArrowLeft, CalendarDays, Check, DollarSign, MapPin, TicketPercent, CreditCard, X, Banknote } from "lucide-react";
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6 px-4 py-6 sm:px-6">
      <div className="flex items-center gap-3">
        <Link to={isOwner ? "/booking/manage" : "/booking/my-bookings"}>
          <Button variant="ghost" size="sm"><ArrowLeft className="h-4 w-4" /> Quay lại</Button>
        </Link>
      </div>

      <section>
        <p className="text-sm font-semibold uppercase tracking-[0.14em] text-brand-700">{isOwner ? "Owner" : "Customer"}</p>
        <div className="flex items-center gap-3">
      ...
      {isCustomer && booking.status === "Approved" && (
        <Card className="space-y-4 rounded-md p-5">
      ...
      <Card className="space-y-4 rounded-md p-5">
        ...
        {booking.customerNote && (
          <div>
            <p className="text-xs text-slate-500">{isOwner ? "Ghi chú khách hàng" : "Ghi chú"}</p>
