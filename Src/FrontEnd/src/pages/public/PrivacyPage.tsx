...
export default function PrivacyPage() {
  ...
  useEffect(() => {
    getCmsPageBySlug("privacy-policy").then((data) => {
      if (data?.content) {
        try {
    ...
    <div className="bg-gradient-to-br from-[#faf7ff] via-white to-[#f5efff] text-slate-900 dark:from-[#0e0720] dark:via-black dark:to-[#05030f] dark:text-white min-h-screen transition-colors duration-300">
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <p className="text-sm sm:text-base font-black uppercase tracking-[0.25em] text-brand-600 dark:text-brand-400">Bảo mật</p>
            <h1 className="mt-2 text-4xl font-black uppercase tracking-tight sm:text-5xl">
              Chính sách{" "}
              <span className="italic lowercase text-brand-600 dark:text-brand-400">bảo mật</span>
            </h1>
          </div>
        </div>
