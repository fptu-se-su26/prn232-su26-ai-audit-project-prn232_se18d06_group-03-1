      ...
      {showFilters && (
        <div className="flex flex-wrap items-center gap-3 border-b border-slate-100 bg-slate-50 px-4 py-3">
          ...
          {(!typeFilter || typeFilter === "Motorbike") && (
            <>
              <FilterDropdown label="Loại xe máy" value={bikeTypeFilter} onChange={(v) => { setBikeTypeFilter(v); setPage(1); void load(1, keyword, sortBy, typeFilter, brandFilter, modelFilter, fuelTypeFilter, seatCountFilter, transmissionFilter, bodyTypeFilter, v, engineCapacityFilter); }}
                options={[{ value: "", label: "Loại xe máy" }, ...motorbikeTypeOptions]} />
              <input type="text" value={engineCapacityFilter} onChange={(e) => { setEngineCapacityFilter(e.target.value); setPage(1); void load(1, keyword, sortBy, typeFilter, brandFilter, modelFilter, fuelTypeFilter, seatCountFilter, transmissionFilter, bodyTypeFilter, bikeTypeFilter, e.target.value); }} placeholder="Dung tích, VD: 125cc" className="h-8 w-32 rounded-md border border-slate-300 bg-white px-2.5 text-sm outline-none placeholder:text-slate-400 focus:border-brand-500 focus:ring-1 focus:ring-brand-500" />
            </>
          )}
      ...
      <div className="p-4">
        {isLoading ? (
          ...
          <div className="flex min-h-[300px] flex-col items-center justify-center text-center">
            ...
            <p className="mt-3 text-sm text-slate-500">Chưa có xe nào đang cho thuê</p>
          </div>
        ) : (
