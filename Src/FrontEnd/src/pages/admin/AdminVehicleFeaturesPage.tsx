...
import { getVehicleFeatures, createVehicleFeature, updateVehicleFeature, deleteVehicleFeature } from "@/features/vehicleFeatures/services/vehicleFeatureService";
import type { VehicleFeatureResponse } from "@/features/vehicleFeatures/types";
...
export default function AdminVehicleFeaturesPage() {
  const [items, setItems] = useState<VehicleFeatureResponse[]>([]);
  ...
  const [editItem, setEditItem] = useState<VehicleFeatureResponse | null>(null);
  ...
  const load = useCallback(async (p: number, kw: string, sort: string, vt: string) => {
    ...
    try {
      const result = await getVehicleFeatures({ page: p, pageSize: PAGE_SIZE, keyword: kw || undefined, sortBy: sort || undefined, vehicleType: vt || undefined });
      setItems(result.items); setTotalCount(result.totalCount); setPage(result.page); setTotalPages(result.totalPages);
    } catch { setError("Không thể tải danh sách tính năng xe."); }
  ...
  function openEdit(item: VehicleFeatureResponse) { setEditItem(item); setFormName(item.name); setFormVehicleType(item.vehicleType === "Motorcycle" ? "Motorbike" : item.vehicleType); setFormError(""); setModalOpen(true); }

  async function handleSave() {
    if (!formName.trim()) { setFormError("Vui lòng nhập tên tính năng."); return; }
    setSaving(true); setFormError("");
    try {
      if (editItem) {
        await updateVehicleFeature(editItem.id, { name: formName.trim(), vehicleType: formVehicleType, isActive: editItem.isActive });
      } else {
        await createVehicleFeature({ name: formName.trim(), vehicleType: formVehicleType });
      }
  ...
  async function handleToggleActive(item: VehicleFeatureResponse) {
    await updateVehicleFeature(item.id, { name: item.name, vehicleType: item.vehicleType, isActive: !item.isActive });
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-slate-950">Tính năng xe</h1>
          <p className="mt-1 text-sm text-slate-500">Quản lý danh sách tính năng xe ô tô và xe máy.</p>
        </div>
        <Button onClick={openCreate}><Plus className="h-4 w-4" /> Thêm tính năng</Button>
      </div>
      ...
      <div className="overflow-hidden rounded-md border border-slate-200 bg-white">
        <div className="flex flex-wrap items-center gap-2 border-b border-slate-200 px-4 py-3">
          <div className="relative flex-1 sm:max-w-xs">
            ...
            <input ref={searchRef} type="text" value={keyword} onChange={(e) => setKeyword(e.target.value)} onKeyDown={handleKeyDown}
              placeholder="Tìm tên tính năng..." className="h-9 w-full rounded-md border border-slate-300 bg-white pl-9 pr-3 text-sm text-slate-900 placeholder-slate-400 outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500" />
          </div>
        ...
        <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
          <div className="text-sm font-medium text-slate-700">{totalCount} tính năng</div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-200 text-sm">
            <thead className="bg-slate-50 text-left text-xs font-semibold uppercase text-slate-500">
              <tr><th className="px-4 py-3">Tên tính năng</th><th className="px-4 py-3">Loại xe</th><th className="px-4 py-3">Trạng thái</th><th className="px-4 py-3">Thao tác</th></tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              ...
              {!isLoading && items.length === 0 && (
                <tr><td colSpan={4} className="px-4 py-10 text-center text-sm text-slate-500">Không có tính năng nào.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? "Sửa tính năng" : "Thêm tính năng"}>
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-700">Tên tính năng</label>
