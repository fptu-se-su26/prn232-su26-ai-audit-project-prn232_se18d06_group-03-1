...
import { fuelTypeOptions, getFuelTypeLabel } from "@/features/vehicleModelVariants/options";
...
export default function AdminCarVariantsPage() {
  ...
  const [formSeatCount, setFormSeatCount] = useState("");
  ...
  const [formDrivetrain, setFormDrivetrain] = useState("");
  ...
  const load = useCallback(async (p: number, kw = keyword) => {
    ...
    try {
      const result = await getVehicleModelVariants({
        ...
        seatCount: filterSeatCount || undefined,
  ...
  function openCreate() {
    setEditItem(null); setSelectedBrandId(""); setModels([]); setFormModelId(""); setFormName(""); setFormSeatCount(""); setFormTransmission(""); setFormFuelType(""); setFormBodyType(""); setFormDrivetrain(""); setFormRequiredLicenseClassId(""); setFormError(""); setModalOpen(true);
  }
  function openEdit(item: VehicleModelVariantResponse) {
    setEditItem(item); setFormModelId(String(item.modelId)); setFormName(item.name); setFormSeatCount(item.seatCount != null ? String(item.seatCount) : ""); setFormTransmission(item.transmission ?? ""); setFormFuelType(item.fuelType ?? ""); setFormBodyType(item.bodyType ?? ""); setFormDrivetrain(item.drivetrain ?? ""); setFormRequiredLicenseClassId(item.requiredLicenseClassId != null ? String(item.requiredLicenseClassId) : ""); setFormError(""); setModalOpen(true);
  ...
  async function handleSave() {
    ...
    try {
      const data = {
        modelId: Number(formModelId), name: formName.trim(), vehicleType: "Car",
        seatCount: formSeatCount ? Number(formSeatCount) : null,
        transmission: formTransmission || null, fuelType: formFuelType || null,
        bodyType: formBodyType || null, drivetrain: formDrivetrain || null,
        bikeType: null, engineCapacity: null,
  ...
  async function handleToggleActive(item: VehicleModelVariantResponse) {
    await updateVehicleModelVariant(item.id, {
      modelId: item.modelId, name: item.name, vehicleType: "Car",
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div><h1 className="text-2xl font-semibold text-slate-950">Phiên bản ô tô</h1><p className="mt-1 text-sm text-slate-500">Quản lý các phiên bản ô tô theo dòng xe.</p></div>
        ...
        {showFilters && (
          <div className="flex flex-wrap items-center gap-3 border-b border-slate-100 bg-slate-50 px-4 py-3">
            <FilterDropdown label="Hãng" value={filterBrandId} onChange={(v) => { setFilterBrandId(v); setFilterModelId(""); setPage(1); }}
              options={[{ value: "", label: "Tất cả hãng" }, ...brands.filter((b) => b.vehicleType === "Car").map((b) => ({ value: String(b.id), label: b.name }))]} />
            ...
            <FilterDropdown label="Số chỗ" value={filterSeatCount} onChange={(v) => { setFilterSeatCount(v); setPage(1); }}
            ...
            <FilterDropdown label="Kiểu thân" value={filterBodyType} onChange={(v) => { setFilterBodyType(v); setPage(1); }}
              options={[{ value: "", label: "Kiểu thân" }, ...carBodyTypes.map((item) => ({ value: item, label: item }))]} />
            <FilterDropdown label="Truyền động" value={filterTransmission} onChange={(v) => { setFilterTransmission(v); setPage(1); }}
            ...
            <tbody className="divide-y divide-slate-100">
              {items.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-slate-600">{item.transmission ?? "-"}</td>
        ...
        <div className="popup-scrollbar max-h-[70vh] space-y-4 overflow-y-auto">
          <div>
            ...
            <select value={selectedBrandId} onChange={(e) => void loadModelsByBrand(e.target.value)} className="mt-1 h-10 w-full rounded-md border border-slate-300 px-3 text-sm outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500">
              ...
              {brands.filter((b) => b.vehicleType === "Car").map((b) => <option key={b.id} value={b.id}>{b.name}</option>)}
            </select>
          </div>
          <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-slate-700">Số ghế</label>
              <input type="number" min={1} max={99} value={formSeatCount} onChange={(e) => setFormSeatCount(e.target.value)} className="mt-1 h-10 w-full rounded-md border border-slate-300 px-3 text-sm outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700">Truyền động</label>
              <select value={formTransmission} onChange={(e) => setFormTransmission(e.target.value)} className="mt-1 h-10 w-full rounded-md border border-slate-300 px-3 text-sm outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500">
                <option value="">Chọn</option><option value="Automatic">Tự động</option><option value="Manual">Số sàn</option><option value="CVT">CVT</option><option value="DCT">DCT</option>
              </select>
            </div>
            <div>
              ...
              <select value={formFuelType} onChange={(e) => setFormFuelType(e.target.value)} className="mt-1 h-10 w-full rounded-md border border-slate-300 px-3 text-sm outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500">
                <option value="">Chọn</option>{fuelTypeOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700">Kiểu thân xe</label>
              <select value={formBodyType} onChange={(e) => setFormBodyType(e.target.value)} className="mt-1 h-10 w-full rounded-md border border-slate-300 px-3 text-sm outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500">
                <option value="">Chọn</option><option value="Sedan">Sedan</option><option value="SUV">SUV</option><option value="Hatchback">Hatchback</option><option value="Coupe">Coupe</option><option value="Convertible">Mui trần</option><option value="Pickup">Bán tải</option><option value="MPV/Minivan">MPV/Minivan</option><option value="Wagon">Wagon</option>
              </select>
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700">Dẫn động</label>
              <select value={formDrivetrain} onChange={(e) => setFormDrivetrain(e.target.value)} className="mt-1 h-10 w-full rounded-md border border-slate-300 px-3 text-sm outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500">
                <option value="">Chọn</option><option value="FWD">FWD</option><option value="RWD">RWD</option><option value="AWD">AWD</option><option value="4WD">4WD</option>
              </select>
            </div>
            <div>
