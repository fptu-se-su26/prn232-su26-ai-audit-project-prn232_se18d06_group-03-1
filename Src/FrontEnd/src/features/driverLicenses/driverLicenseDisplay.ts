...
export function translateDriverLicenseMessage(message?: string | null, fallback = "Vui lòng kiểm tra lại ảnh GPLX.") {
  ...
  if (normalized.includes("gplx") && normalized.includes("a1") && normalized.includes("xác minh")) {
    return "Hạng GPLX A1 không phù hợp để xác minh ô tô.";
  }
  if (normalized.includes("gplx") && normalized.includes("không phù hợp")) {
  ...
  if (normalized.includes("driver license upload is temporarily locked") || (normalized.includes("gplx") && normalized.includes("quá nhiều"))) {
    return "Bạn đã upload GPLX thất bại quá nhiều lần. Vui lòng chờ hết thời gian khóa rồi thử lại.";
  }
  if (normalized.includes("driver license can only be updated") || normalized.includes("cập nhật lại sau")) {
