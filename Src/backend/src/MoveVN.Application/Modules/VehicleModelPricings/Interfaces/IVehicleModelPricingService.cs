...
{
    Task<PagedResult<VehicleModelPricingResponse>> GetAllAsync(string? keyword, string? sortBy, string? vehicleType, int? brandId, int? modelId, bool? isActive, decimal? minPrice, decimal? maxPrice, int page, int pageSize, CancellationToken cancellationToken = default);
    Task<VehicleModelPricingResponse> GetByIdAsync(int id, CancellationToken cancellationToken = default);
    Task<List<RegionPriceResponse>> GetRegionPricesByModelAsync(int modelId, CancellationToken cancellationToken = default);
